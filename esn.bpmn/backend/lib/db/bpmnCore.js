 ...
 * pubsub notification.
 *
 * @param {object} bpmn             The bpmn to be saved in the database.
...
function save(bpmn, callback) {
  ...
  bpmnData.save(function(err, res) {
    if (!err) {
      logger.info('Added new event message in database:', { _id: res._id.toString() });
    } else {
...
function findBpmnById(id, callback) {
  BpmnModel.findOne({_id: id}, callback);
}
